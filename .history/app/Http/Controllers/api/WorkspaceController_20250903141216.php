...
class WorkspaceController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $ownedWorkspaces = Workspace::where('owner_id', $user->id)
            ->with(['owner', 'teams'])
            ->get();
            
        // Get workspaces where user is a member via pivot table
        $memberWorkspaces = Workspace::whereHas('users', function ($query) use ($user) {
                $query->where('user_id', $user->id);
            })
            ->with(['owner', 'teams'])
            ->get();
            
        $workspaces = $ownedWorkspaces->merge($memberWorkspaces)->unique('id');
