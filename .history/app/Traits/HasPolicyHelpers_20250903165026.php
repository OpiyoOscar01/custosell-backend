...
trait HasPolicyHelpers
{
    ...
    {
        $user = Auth::user();
        return $user && method_exists($user, 'can') && $user->can('bulk-actions');
    }
    ...
    {
        $user = Auth::user();
        return $user && method_exists($user, 'hasAnyRole') && $user->hasAnyRole(['admin', 'manager']);
    }
    ...
    protected function hasPermissionAndRole(string $permission, array $allowedRoles = ['admin', 'manager']): bool
    {
        ...
        $hasRole = method_exists($user, 'hasAnyRole') && $user->hasAnyRole($allowedRoles);
